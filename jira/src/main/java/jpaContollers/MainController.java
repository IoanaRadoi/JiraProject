...
import pojo.RowTotalPerReleaseSprint;
import pojo.Sprint;
...
public class MainController {
    ...
    public List<Sprint> getSprintsByRelease(Long idProjectRelease) {
        ...
        for (ProjectreleasesprintDB sprintReleaseProject : sprintsFromDatabase) {
            ...
            sprintsFound.add(new Sprint(sprintFound.getId(), sprintFound.getName(), sprintReleaseProject.getCapacity(), sprintReleaseProject.getId().toString()));
        }
        return sprintsFound;
    }
    ...
    public List<Project> getProjectsByAccount(Long idAccount) {
        ...
        for (AccountprojectsDB apdb : accountprojectsFromDatabase) {
            projects.add(new Project(apdb.getAccountproject(), getReleasesByProject(apdb.getAccountproject()), projectJpaController.findProjectDB(apdb.getAccountproject()).getName(), getSprintsByRelease(apdb.getAccountproject())));
        }
        return projects;
    ...
    public List<Project> getAllProjects() {
        ...
        for (JiraprojectDB pdb : projectsFromDatabase) {
            ...
            List<Sprint> sprintsFound = getSprintsByRelease(pdb.getId());
            projects.add(new Project(pdb.getId(), releasesFound, pdb.getName(), sprintsFound));
        }
        return projects;
    ...
    public RowTotalPerReleaseSprint getRowSprintIfSprintsAlreadyExists(List<RowTotalPerReleaseSprint> rowSprints, String idProjectReleaseSprint, String idSprint) {
        for (RowTotalPerReleaseSprint perReleaseSprint : rowSprints) {

            if (perReleaseSprint.getIdSprintReleaseProject().equals(idProjectReleaseSprint) && perReleaseSprint.getIdSprint().equals(idSprint)) {
                return perReleaseSprint;
    ...
    public List<RowTotalPerReleaseSprint> getProjectRelesesPerReleaseSprintFromStoryPointsSelected(List<String> projectsSelectedIds, List<String> releasesSelectedIds) {
        List<StorypointDB> storyPoints = getRowRaport2(projectsSelectedIds, releasesSelectedIds);

        List<RowTotalPerReleaseSprint> rowTotalPerReleaseSprint = new ArrayList<>();
        ...
        for (StorypointDB storyPointDB : storyPoints) {
            ...
            RowTotalPerReleaseSprint rez = getRowSprintIfSprintsAlreadyExists(rowTotalPerReleaseSprint, String.valueOf(projectreleasesprintDB.getId()), String.valueOf(projectreleasesprintDB.getIdsprint()));

            if (rez != null) {
                Long total = Long.valueOf(rez.getTotalPerReleaseSprint()) + Long.valueOf(storyPointDB.getStorypoint());
                rez.setTotalPerReleaseSprint(total.toString());
                ...
                rowTotalPerReleaseSprint.add(new RowTotalPerReleaseSprint(projectreleaseDB.getId(), projectreleaseDB.getAn(), jirareleaseDB.getName(), jiraprojectDB.getName(), storyPointDB.getStorypoint(), String.valueOf(projectreleasesprintDB.getIdsprint()), String.valueOf(storyPointDB.getIdprojectreleasesprint()), projectreleasesprintDB.getCapacity()));
            }
        }
        return rowTotalPerReleaseSprint;
    }
    ...
    public CategoryDetailsBySprint getCategoryDetailIfCategoryAlreadyExists(List<CategoryDetailsBySprint> result, String idCategory) {
        for (CategoryDetailsBySprint idbs : result) {
            if (idbs.getIdCategory().equals(idCategory)) {
                return idbs;
            }
        }
        return null;
    ...
    public List<CategoryDetailsBySprint> getAllStoryPointsByProjectReleaseSprint(Long idProjectReleaseSprintId) {
        List<StorypointDB> storypointDBs = storypointJpaController.findByProjectReleaseSprintId(idProjectReleaseSprintId);
        List<CategoryDetailsBySprint> result = new ArrayList<>();

        for (StorypointDB storypointDB : storypointDBs) {
            CategoryDetailsBySprint categoryDetailsBySprint = getCategoryDetailIfCategoryAlreadyExists(result, String.valueOf(storypointDB.getIdefforttype()));

            if (categoryDetailsBySprint != null) {
                Long total = Long.valueOf(categoryDetailsBySprint.getTotalStoryPoints()) + Long.valueOf(storypointDB.getStorypoint());
                categoryDetailsBySprint.setTotalStoryPoints(total.toString());

            } else {
                result.add(new CategoryDetailsBySprint(String.valueOf(storypointDB.getIdefforttype()), storypointDB.getStorypoint()));
            }
        }
        return result;
    }
    ...
    public String getNameItemByIdCategory(Long idCategory){
        return efforttypeDBJpaController.findEfforttypeDB(idCategory).getName();
